...
public class StatusBarUtil {
    ...
    /**
     * 使状态栏半透明，适用于图片作为背景的界面,此时需要图片填充到状态栏
     * @param activity 需要设置的activity
     */
     ...
     * @param activity     需要设置的activity
     * @param drawerLayout DrawerLayout，传入的值必须为(DrawerLayout)findViewByID(R.id.....)
     * @param color        状态栏颜色值
     * 内容布局需要对其子布局设置padding top值，否则仿状态栏色块会被遮挡在最下面，布局内容延伸到状态栏。根布局必须android:fitsSystemWindows="true"
     */
...
//         内容布局不是 LinearLayout 时,设置padding top
//        if (!(contentLayout instanceof LinearLayout) && contentLayout.getChildAt(1) != null) {
        contentLayout.getChildAt(1).setPadding(0, getStatusBarHeight(activity), 0, 0);  //给内容布局设置padding top
//        }
        ...
        contentLayout.setClipToPadding(true);
        drawer.setFitsSystemWindows(false);
        drawer.getChildAt(1).setPadding(0, getStatusBarHeight(activity), 0, 0);  //给抽屉设置padding top
